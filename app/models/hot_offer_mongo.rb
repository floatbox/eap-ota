...
class   HotOfferMongo
  ...
  include Mongoid::Document
    ...
    def self.featured code=nil
      ...
      offers = HotOfferMongo.where(:for_stats_only => false ).and(:price_variation.gt => 0).order_by(:created_at => :desc).limit(30)
    ...
    def set_some_vars
      if @search and @recommendation
          ...
          self.destination = DestinationMongo.find_or_create_by(:from_iata => @search.segments[0].from_as_object.iata, :to_iata => @search.segments[0].to_as_object.iata, :rt => @search.rt)
