...
guard 'rspec', :cli => "--drb", :version => 2 do
  ...
  watch(%r{^lib/(.+)\.rb$})                           { |m| ["spec/lib/#{m[1]}_spec.rb", "spec/lib/#{ m[1].gsub('/', '_') }_spec.rb"] }
  watch(%r{^lib/strategy/([^/]+)/(.+)\.rb$})          { |m| "spec/lib/strategy_#{m[1]}_spec.rb" } # strategy/amadeus_spec, sirena_spec
