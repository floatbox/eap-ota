...
example 'svocdg'
agent    "До 30.06.11г. 7% от всех опубл. тарифов на рейсы 6H (В договоре Interline отдельно не прописан.)"
agent    "С 01.07.11г. 5% от всех опубл. тарифов на рейсы 6H (В договоре Interline отдельно не прописан.)"
subagent "До 30.06.11г. 5% от опубл. тарифов на собств.рейсы 6H"
...
example "svomxp mxpsvo"
example "svomxp"
example "mxpsvo"
agent "C 01.05.11г. по 30.06.11г"
agent "5% от опубл. тарифов Эконом класса на собств. рейсы AZ"
agent "Условия предоставления комиссии: "
agent "Период действия комиссии с 1 апреля по 31 мая 2011 года "
agent "Комиссия предоставляется на билеты, имеющие период путешествия с 1 апреля 2011 года по 31 декабря 2011 года "
agent "Комиссия предоставляется на собственные рейсы Alitalia 055 ( AZ/AP/XM/VE) на участках Россия-Италия-Россия (code-share рейсы на данных участках исключены). На остальных участках code-share рейсы разрешены под кодом Alitalia. " 
agent "Комиссия не предоставляется на билеты,  которые оформляются отдельной перевозкой  на   внутренние  рейсы по Италии (К примеру, Рим-Пиза или Рим-Пиза-Рим) , туроператорские и групповые тарифы ."
agent "Кредитовая корректировка (ACM) при не внесении комиссии в момент продажи авиабилета запрещена. (Если агентство не потребовало комиссию в момент продажи билета, эту комиссию НЕЛЬЗЯ потребовать задним числом позже)."
subagent "3% (тарифы эконом-класса)"
subagent "• Период действия комиссии с 1 мая по 31 мая 2011 года"
subagent "• Комиссия предоставляется на билеты, имеющие период путешествия с 01.04.11г. по 31.12.11г."
subagent "• Комиссия предоставляется на собственные рейсы Alitalia (AZ/AP/XM/VE) на участках Россия-Италия-"
subagent "Россия (code-share рейсы на данных участках исключены). На остальных участках code-share рейсы разрешены под кодом Alitalia."
subagent "• Комиссия не предоставляется на билеты, которые оформляются отдельной перевозкой на внутренние рейсы по Италии (К примеру, Рим-Пиза или Рим-Пиза-Рим),"
subagent "• Комиссиянепредоставляетсянатуроператорскиеигрупповыетарифы."
subagent "Кредитовая корректировка (ACM) при не внесении комиссии в момент продажи авиабилета запрещена. (Если агентство не потребовало комиссию в момент продажи билета, эту комиссию НЕЛЬЗЯ потребовать задним числом позже)."
classes :economy
check { (country_iatas.first == 'RU' && country_iatas.include?('IT')) || (country_iatas.first == 'IT' && country_iatas.include?('RU')) }
important!
commission "5%/3%" #увеличился срок

example 'svocdg/business'
agent "7% от опубл. тарифов Бизнес класса на собств. рейсы AZ"
subagent "5% (тарифы бизнес-класса)."
classes :business
important!
commission "7%/5%" # и здесь
...
agent   "C 05.05.2011 по 31.03.2012г."
agent   "по маршрутам Москва-Пекин или Москва-Пекин-Москва:"
...
#разбирать
subagent "С 01.04.2011г. 3,5% от опубл. тарифов на собств.рейсы CA с началом перевозки из России; 0% на все остальные тарифы."
...
interline :possible
disabled "дописать гео-правило"
commission "0%/0%"

#example 'svoflu flusvo/ab'
agent "по 30.06.2011 г. "
agent "к стандартной комиссии БСП 1%  добавляет 7% по направлениям Russian Federation: WORLD, при этом перевозка может быть завершена в европе или китае "
...
subagent "До 30.06.2011г. к стандартной комиссии 5 руб. выплачивается доп.комиссия 5%"
...
disabled "по всем классам кроме Т, дописать фильтр, как насчет складывания рублей и процентов?"
commission "8%/5%" #+5 рублей мы еще должны получить, но хуй.
...
example 'szxcdg'
agent    "с 01.11.2011 9% от всех опубл. тарифов на рейсы HU (В договоре Interline не прописан.)"
subagent "7% от всех опубл. тарифов на рейсы HU (В договоре Interline не прописан.)"
interline :possible
disabled 
commission "9%/7%"

example 'svocdg' #FIX new rules
agent    "На период до 31.10.2011г. на собств.рейсы HU по опубл. тарифам:"
...
agent    "Отдельные перелеты с вылетами из Пекина по Китаю на собств. рейсы HU по опубл. тарифам 0% "
agent    "С 01.11.2011г. "
agent    "Отдельные перелеты с вылетами из Пекина по Китаю на собственные рейсы HU по опубл. тарифам 0% "
subagent "На период до 30.04.2011г. на собств.рейсы HU по опубл. тарифам:"
...
subagent "С 01.05.2011г."
subagent "Отдельные перелеты с вылетами из Пекина по Китаю на собственные рейсы HU по опубликованным тарифам 0%"
international
disabled "надо бы поразбивать на отдельные класс-профилированные правила"
commission "9%/6.3%"

#Москва-китай, изменения от 17 мая
...
agent "9% перелет с 05.05.11-31.10.11 / class Z of the flight MOW - CHINA или MOW - CHINA - MOW"
agent "9% перелет с 15.05.11-31.10.11с вылетами из городов KGA OVB IKT на все классы"
agent "0% по опубл. тарифам отдельные перелеты с вылетами из Пекина по Китаю на собств. рейсы HU"
agent "С 01.11.2011г. 9% от всех опубл. тарифов на рейсы HU (В договоре Interline не прописан.)"
agent "0% по опубл. тарифам отдельные перелеты с вылетами из Пекина по Китаю на собств. рейсы HU"
subagent "18% перелет с 01.01.11-31.10.11 / class F, P, C of the flight MOW - CHINA или MOW - CHINA - MOW 13% перелет с 01.01.11-31.10.11 / class D, I, J of the flight MOW - CHINA или MOW - CHINA - MOW 10% перелет с 20.05.11-31.05.11 / class Y,B,H,K,L,M,Q,X,V,T,S,N,U,O of the flight MOW - CHINA или MOW - CHINA - MOW
7% перелет с 01.01.11-31.10.11 / class Y,B,H,K,L,M,Q,W,S,U,E,O of the flight MOW - CHINA или MOW - CHINA - MOW 7% перелет с 05.05.11-31.10.11 / class Z of the flight MOW - CHINA или MOW - CHINA - MOW *********
7% перелет с 15.05.11-31.10.11с началом перевозки из городов KJA OVB IKT / all class ********* 1% начало перелета из третьей страны в Китай на все классы *********
1% перелет с 05.05.11 по 31.10.11г./class F, P, C , D , I , J of the flight CHINA - MOW или CHINA - MOW - CHINA 1% перелет с 05.05.11 по 31.10.11г./class Y,B,H,K,L,M,Q,X,V,T,S,N,U, of the flight CHINA - MOW или CHINA - MOW - CHINA
********* 0% по опубл. тарифам отдельные перелеты с вылетами из Пекина по Китаю на собств. рейсы HU *********С 01.11.2011г. 7% от опубл. тарифов на собств. рейсы HU 0% по опубл. тарифам отдельные перелеты с вылетами из Пекина по Китаю на собств. рейсы HU"
#todo
disabled
...
subagent "До 30.06.2011г. 5% от опубл. тарифов по классам бронирования: M, H, Q, V, W, S, L на собств. рейсы UA из России в США с обяз. уч. трансатлантич. рейсов UA965/UA964."
#subclasses 'MHQVWSL'
...
subagent "До 30.06.2011г. 7% от опубл. тарифов по классам бронирования: F, A, J, C, D, Z, Y, B на собств. рейсы UA из России в США с обязат. уч. трансатлантич. рейсов UA965/UA964."
important!
subclasses 'FAJCDZYB'
check { (flights.every.full_flight_number & %W(UA965 UA964)).any? }
commission "9%/7%"
...
example 'svocdg'
agent "1% от всех опубл. тарифов на собств.рейсы авиакомпании. (Interline отдельно не прописан)"
subagent "5 руб. с билета по опубл. тарифам на собств. рейсы 5L"
commission "1%/5"
...
agent "1% от всех опубл. и конфиде. тарифов на междунар. и внутр. рейсы A3. Билеты по Interline могут быть выписаны только при условии присутствия сегментов A3"
subagent "5 руб с билета по всем опубл. и конфиде. тарифов на междунар. и внутр. рейсы A3. Билеты по Interline могут быть выписаны только при условии присутствия сегментов A3."
interline :possible
commission "1%/5"
