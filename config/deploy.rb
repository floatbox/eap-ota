...
namespace :deploy do
  ...
  task :restart_delayed_job, :roles => :daemons, :on_no_matching_servers => :continue do
    DJ_QUEUES.each { |queue| run "sudo /usr/bin/sv restart /etc/service/delayed_job_#{queue} -w 15" }
  end
  ...
  task :stop_delayed_job, :roles => :daemons, :on_no_matching_servers => :continue do
    DJ_QUEUES.each { |queue| run "sudo /usr/bin/sv stop /etc/service/delayed_job_#{queue} -w 15" }
  end
