...
describe PayuCharge do
  ...
  describe "factories" do
    specify { build(:payu_charge).should be_valid }
    specify { build(:payu_charge, :charged).should be_valid }
    specify { create(:payu_charge, :charged).should be_charged }
    specify { create(:payu_charge, :charged).should_not be_new_record }
  end
  ...
  describe "commission" do
    ...
    it 'should recalculate earnings on save' do
      p = PayuCharge.create! :commission => '1.23%', :price => 1000
      p.commission.should == Fx('1.23%')
      p.earnings.should == 1000 - 12.3
    end

    it 'probably should get commission from Conf, if unset' do
      Conf.payu.stub(:commission).and_return('3.45%')
      p = PayuCharge.create! :price => 1000
  ...
  it 'should not allow to update price on charged payment' do
    charge = create(:payu_charge, :charged, :price => 1000)
  ...
  describe "state" do
    subject {PayuCharge.new(:status => current_state)}
